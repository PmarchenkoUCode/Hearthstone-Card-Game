...
module.exports = function (CardContainer) {
	...
	for (var i = 0; i < 60; i++) {
		...
		for (let key in cardArray) {
			if (cardArray[key].name) {
				let card = new Card("card nr: "+ i, cardArray[key].ico, cardImage, cardArray[key].name, cardArray[key].attack, cardArray[key].health, cardArray[key].cost, randomcharge, randomsummon, randomtaunt);
